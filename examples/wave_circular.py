...
from library.io import read_mesh_flat, read_data_flat
...
if __name__ == '__main__':
    ...
    geometry, topology = read_mesh_flat(root)
    POINTS = len(geometry[0])
    u = read_data_flat(root, 'u.h5', len(geometry), NT)
    ...
    #     u_grid = interpolate(geometry, u, X, Y, method)
    #     Xi = solve_system(u_grid, axes, intervals, polynomial_order=2, derivative_order=2)
    #
    #     print(
    #         f"Interpolation method: {methods}. Original number of points: {len(u)}. Interpolation points: {(NX + 1) * (NY + 1)}")
    #     print(Xi)
    ...
    for function in functions:
        u_grid = interpolate_rbf(u, geometry, X, Y, dimensions, function)
        Xi = solve_system(u_grid, axes, intervals, polynomial_order=2, derivative_order=2)

        print(
            f"Interpolation method: {function}. Original number of points: {len(u)}. Interpolation points: {(NX + 1) * (NY + 1)}")
        print(Xi)
        print_equation(Xi, terms)
        print()
    ...
    # image_dir = '../images/rbf_images_circular'
    ...
    #     create_interpolation_plot(u[:, i], geometry, t[i], x, y, X, Y, dimensions, 'multiquadric', filename)
    #     filenames.append(filename)
    #
    # # Create a GIF from the saved images
    # with imageio.get_writer(os.path.join('../images', 'rbf_interpolation_circular_shift.gif'), mode='I',
    ...
    #     os.remove(filename)
    #
    # print("GIF created: rbf_interpolation_circular.gif")


# TODO remove also the folder
# TODO solo i punti dento (usando solo i punti e acerando los puntos en el externo)
#      usando solo i punti: - integrale triangoli su punti di gauss - integrale quadratto metodo del trapezio (SOLO QUESTO)
#      con tutti i quadratti (anche random)
# TODO una percentuali dei punti dentro per l'interpolazione
# TODO posizionamento random di Dx, Dy e Dt assegnati
# TODO prova senza itegrale (sindy liscio)
# TODO plot afiancati reale e interpolati